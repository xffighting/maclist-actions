...
4. 直接输入模糊文件名、项目名或路径片段，原位置立即展开候选清单，显示文件名和所在目录。
5. 用方向键或鼠标选择候选并按回车，文件在原上传窗口里被精确定位并选中。开发预览阶段由你最后点击“打开”。
...
| 可见候选清单与搜索状态 | 已实现 | 加载、结果、无匹配、失败状态齐全；旧查询会取消 |
| Spotlight 本地元数据候选 | 已实现 | 只使用文件名和路径元数据，不读正文或候选文件 |
